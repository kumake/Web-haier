						...
						<tr>
							...
							<div id="neiye_main">
								<!--内容开始 -->
										...
										if total = 0 then 
										Dim Tarray:Tarray = UICon.QueryData("select count(*) as total from user_cjwt where "&strwhere&"")
										...
										Dim sql:sql = UICon.QueryPageByNotIn("*","user_cjwt", ""&strwhere&"", "ID", "indexID desc,ID",false,current_page,pagesize)
										'response.Write sql
										'response.End()
										...
										<span style="font-family:'宋体';">·</span><a href="cjwt_in.asp?categoryid=<%=rsn("categoryID")%>&amp;itemid=<%=rsn("id")%>" class="y_0"><%=rsn("title")%></a> </td>
										</tr>
										<tr>
